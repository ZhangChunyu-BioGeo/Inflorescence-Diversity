# =========================================================================
# Process Observed (Actual) Data in a Loop and Save Model Results
...
# Define the range of seeds for the loop (to create 100 different train/test splits)
seed_start <- 1
seed_end <- 100

# Define input data path and the root path for output results
input_data_path <- "MLModel/ObData/ObData.csv"
output_root_path <- "MLModel/ObModel/"

# Read the observed data
original_df <- read.csv(file = input_data_path, header = TRUE)
...
save_results <- function(feature_importance_IMP, feature_importance_SHAP, Model_R2, Model_RMSE, Model_MSE, save_path, models) {
  ...
  cat("    All results have been saved successfully!\n")
}

# ---------------------------- MAIN LOOP ----------------------------
# Loop through each seed to generate different train/test splits
for (seed in seed_start:seed_end) {
  ...
  # Use a fresh copy of the original data for each run
  df <- original_df
  ...
  for (target in target_vars) {
    ...
    # Create train/test split. The seed ensures this split is different for each loop iteration.
    set.seed(MySeed)
